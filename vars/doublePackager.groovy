  ...
  node(envlabel) {
    pipeline {
      ...
      stage('Generating packages list') {
        ...
        retry(3) {
            condaShellCmd("conda install --copy -q -c t/${env.ANACONDA_API_TOKEN}/prometeia/channel/${sourcelabel} -c t/${env.ANACONDA_API_TOKEN}/prometeia -c defaults -c main -c free t/${env.ANACONDA_API_TOKEN}/prometeia/channel/archiviati --override-channels --no-channel-priority -y ${components}", "${CONDAENV}-${envlabel}")
        }
        script {
